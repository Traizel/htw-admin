...
const brightpearlAPI = (options) => {
    console.log(`${options.method} ${options.url}`.yellow);
    return axios({
        ...options
    });
}

const getUnpaidBrightpearlOrders = async (UNPAID_ORDERS_INTERVAL) => {
    const now = new Date();
    const then = new Date();
...
const sendCustomerPaymentToBrightPearl = async cp => {
    ...
    const customerPaymentData = brightpearlAPI(options)
    .then(r => {
        console.log('sendCustomerPaymentToBrightPearl POST result: '.green, r.data);
        return r.data
    })
    .catch(err => {
        console.log(err.message.toString().red);
